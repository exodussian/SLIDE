...
protected:                 //!< protected such that child classes can access the class variables
  ...
  //!< Functions
  ValuePair<> calcSurfaceConcentration(ValuePair<> j, ValuePair<> Dt);
  ValuePair<> calcOverPotential(ValuePair<> cs, double i_app); //!< Should not throw normally, except divide by zero?

  inline double calcArrheniusCoeff() { return (1 / T_ref - 1 / st.T()) / PhyConst::Rg; } //!< Calculates Arrhenius coefficient.

  ValuePair<> calcDiffConstant();        //!< Calculate the diffusion constant at the battery temperature using an Arrhenius relation
